...
class gffb_bestand():
    ...
    def sequentie_uit_bestand3(self):
        bestand = open(self.__bestand3_sequentie.name, "r")
        self.__sequentie = ""
        aantal = 1
        for regel in bestand:
            print(aantal)
            for index in regel:
                if index == "a" or index == "t" or index == "c" or index == "g":
                    self.__sequentie += index
            aantal += 1
        bestand.close()

    def getsequentie(self):
        return self.__bestand2_informatie
...
class gui_maken():
    ...
    def setgegevens_gc_percentage(self):
        gc = round((self.__sequentie.count("c") + self.__sequentie.count("g")) / len(
        self.__sequentie) *100, 2)
        at = round((self.__sequentie.count("a") + self.__sequentie.count("t")) / len(
        self.__sequentie)*100, 2)
        self.__percentage = [gc, at]
        self.__gc = self.__percentage[0] * 10
        self.__at = self.__percentage[1] * 10

    def gui_startscherm(self):
        startscherm = Tk.Tk()
        self.__knop = Tk.Button(heigh=2, master=startscherm, text="klik hier voor tabel met de gevonden patronen",command=self.tabel_patronen)
        ...
        self.__knop5 = Tk.Button(heigh=2, master=startscherm,text="klik hier voor grafiek met GC en AT %  ",command=self.maak_grafiek_percentage)
    ...
    def maak_grafiek_percentage(self):
        grafiek_percentage = Tk.Toplevel()
        upper_frame = Tk.Frame(grafiek_percentage)
        bottom_frame = Tk.Frame(grafiek_percentage)
        upper_frame.pack()
        bottom_frame.pack()
        label = Tk.Label(upper_frame, text='percentage AT/GC %')
        legenda = Tk.Label(bottom_frame, text=("legenda:\n -GC:",self.__percentage[0], "\n-AT: - ", self.__percentage[1]))
        label.pack()
        legenda.pack()
        pie = Tk.Canvas(upper_frame, width=154, height=154)
        pie.pack()
        pie.create_arc((2, 2, 152, 152), fill="red", start=0,
                       extent=(360 * self.__gc) / 1000)
        pie.create_arc((2, 2, 152, 152), fill="blue",
                       start=(360 * self.__gc) / 1000,
                       extent=(360 * self.__at) / 1000)
        quit_button = Tk.Button(heigh=2, width=10, text="return",
